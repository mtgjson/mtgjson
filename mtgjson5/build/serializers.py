...
def serialize_complex_types(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if list_cols:
        for col_name in list_cols:
            inner = schema[col_name].inner  # type: ignore[union-attr]
            if isinstance(inner, pl.Struct):
                batch_fn = _list_of_struct_to_json_batch
            else:
                batch_fn = _list_to_csv_batch
            result = result.with_columns(
                pl.col(col_name).map_batches(batch_fn, return_dtype=pl.String).alias(col_name)
            )
...
def _list_of_struct_to_json_batch(series: pl.Series) -> pl.Series:
    ...
    return pl.Series(
        [
            orjson.dumps([_drop_nulls(item) for item in x]).decode("utf-8")
            if x is not None
            else None
