...
from mtgjson5.mtgjson_models.files import (
    AllPrintingsFile,
    AtomicCardsFile,
    DeckListFile,
    FormatAtomicFile,
...
class JsonOutputBuilder:
    ...
    def _write_all_printings_streaming(
        ...
        with output_path.open("wb") as f:
            ...
            for code, set_data in self.ctx.sets.iter_sets(set_codes=codes):
                ...
                f.write(code.encode())
                f.write(b'":')
                f.write(orjson.dumps(set_data, option=orjson.OPT_SORT_KEYS))
                count += 1
    ...
    def write_format_file(
        ...
        file = FormatPrintingsFile.for_format(format_name, all_printings)
        file.write(output_path)
        return file
    ...
    def write_decks(
        ...
        assembler = self.ctx.deck_assembler(set_codes=set_codes)
        ...
        for deck_raw in decks_df.to_dicts():
            ...
            json_bytes = orjson.dumps(output, option=orjson.OPT_SORT_KEYS)
