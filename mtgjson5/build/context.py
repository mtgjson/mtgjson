...
class AssemblyContext:
    ...
    def from_cache(cls, cache_dir: pathlib.Path | None = None) -> AssemblyContext | None:
        ...
        LOGGER.debug(f"Loaded cached sealed products DataFrame with {len(sealed_df) if sealed_df is not None else 0} rows.")
    ...
    def deck_assembler(self, set_codes: list[str] | None = None) -> DeckAssembler:
        """Create a DeckAssembler with cards loaded from parquet.

        Args:
            set_codes: Optional filter for specific set codes

        Returns:
            DeckAssembler ready to expand deck card references
        """
        from .assemble import DeckAssembler

        return DeckAssembler.from_parquet(self.parquet_dir, set_codes=set_codes)
