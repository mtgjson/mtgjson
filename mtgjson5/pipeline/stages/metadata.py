...
def build_sealed_products_lf(ctx: PipelineContext, _set_code: str | None = None) -> pl.LazyFrame:  # pylint: disable=no-member
    ...
    # A cache written before the token or etched flag existed has no such
    # column, so fall back to a null literal rather than failing the whole build.
    ...
    etched_col = pl.col("etched") if "etched" in contents_cols else pl.lit(None, dtype=pl.Boolean)
    ...
    card_contents = (
        ...
        .agg(
            pl.struct(
                ...
                uuid=pl.col("uuid"),
                foil=pl.col("foil"),
                etched=etched_col.alias("etched"),
