...
def fix_manavalue_for_multiface(lf: pl.LazyFrame) -> pl.LazyFrame:
	...
	# Only update manaValue/convertedManaCost - faceManaValue is correctly set in add_mana_info()
	return lf.with_columns(
...
def join_cardmarket_ids(
	...
	lf = lf.with_columns(pl.coalesce(pl.col("mcmId"), pl.col("cardmarketId").cast(pl.String)).alias("mcmId"))
		...
		face_lookup = (
			...
			.with_columns(
				[
					...
					pl.col("lang").replace_strict(constants.LANGUAGE_MAP, default=pl.col("lang")).alias("language"),
...
def build_sealed_products_lf(ctx: PipelineContext, _set_code: str | None = None) -> pl.LazyFrame:
	"""
		...
		for code, set_data in additional_sets.items():
			...
			if code_upper not in existing_codes:
				new_record = {
					...
					"parentCode": set_data.get("parent_set_code", "").upper()
