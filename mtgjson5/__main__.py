	...
	if sets_to_build or decks_only:
		if args.polars:
			...
			if decks_only:
				...
				results = assemble_with_models(ctx, streaming=True, outputs=outputs_set, pretty=args.pretty)
				LOGGER.info(f"Model assembly results: {results}")
			else:
				assemble_json_outputs(ctx, include_referrals=args.referrals, parallel=True, max_workers=30, pretty=args.pretty)
		else:
