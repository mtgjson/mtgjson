...
class ScryfallProvider(AbstractProvider):
    ...
    SPELLBOOK_SEARCH_URL = "https://api.scryfall.com/cards/search?q=spellbook:%22{}%22&include_extras=true"
    ...
    def download_all_pages_api(
        ...
        while starting_url:
            ...
            starting_url = re.sub(r"&page=\d+", f"&page={page_downloaded}", starting_url, count=1)
    ...
    def _download_api(
        ...
        try:
            ...
            if "504" in response.text:
                ...
                LOGGER.error(f"Unable to convert response to JSON for URL: {url} -> {error}; Message = {response.text}")
    ...
    def download_cards(self, set_code: str) -> list[dict[str, Any]]:
        ...
        scryfall_cards = self.download_all_pages(self.CARDS_URL_ALL_DETAIL_BY_SET_CODE.format(set_code))

        # Return sorted by card name, and by card number if the same name is found
        return sorted(scryfall_cards, key=lambda card: (card["name"], card["collector_number"]))
    ...
    def get_sets_already_built() -> list[str]:
        ...
        """
        json_output_files: list[pathlib.Path] = list(MtgjsonConfig().output_path.glob("**/*.json"))
        ...
        set_codes_found = [
            set_code[:-1] if set_code[:-1] in constants.BAD_FILE_NAMES else set_code for set_code in set_codes_found
        ]
    ...
    def get_sets_to_build(self, args: argparse.Namespace) -> list[str]:
        ...
        # Remove Scryfall token sets (but leave extra sets)
        non_token_sets = {s for s in scryfall_sets if not (s.startswith("T") and s[1:] in scryfall_sets)}
