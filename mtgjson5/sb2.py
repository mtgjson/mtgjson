...
def _get_vectorized_flag() -> bool:
    """Get vectorized flag from config, with env var override for testing."""
    env_override = os.environ.get("MTGJSON_USE_VECTORIZED_PIPELINE")
    if env_override is not None:
        return env_override == "1"
    return MtgjsonConfig().vectorized
...
def parse_printings(oracle_id: Optional[str]) -> List[str]:
    ...
    """
    if not oracle_id or GLOBAL_CACHE.printings_df is None:
        return []

    result = GLOBAL_CACHE.printings_df.filter(pl.col("oracle_id") == oracle_id)
    if result.is_empty():
...
def parse_rulings(oracle_id: Optional[str]) -> List[MtgjsonRulingObject]:
    ...
    """
    if not oracle_id or GLOBAL_CACHE.rulings_df is None:
        return []

    result = GLOBAL_CACHE.rulings_df.filter(pl.col("oracle_id") == oracle_id)
    if result.is_empty():
        return []

    mtgjson_rules: List[MtgjsonRulingObject] = []
    for ruling in result["rulings"][0]:
        # Keys are aliased as "date" and "text" in cache_builder aggregation
        mtgjson_rule = MtgjsonRulingObject(ruling["date"], ruling["text"])
...
def build_mtgjson_set(
    set_code: str, skip_tokens: bool = False
) -> Optional[MtgjsonSetObject]:
    """
    Construct a MTGJSON Magic Set
    :param set_code: Set to construct
    :param skip_tokens: Skip building tokens for this set
    :return: Set object
    """
    ...
    mtgjson_set.mtgo_code = (set_data.get("mtgo_code") or "").upper()
    mtgjson_set.parent_code = (set_data.get("parent_set_code") or "").upper()
    mtgjson_set.block = set_data.get("block") or ""
    mtgjson_set.is_online_only = set_data.get("digital") or ""
    mtgjson_set.is_foil_only = set_data.get("foil_only") or ""
    mtgjson_set.is_non_foil_only = set_data.get("nonfoil_only") or ""
    ...
    # =========================================================================
    # CARD POST-PROCESSING (legacy path only)
    # When vectorized is enabled, ALL card-level post-processing is handled
    # in the DataFrame pipeline within build_base_mtgjson_cards.
    # =========================================================================
    if not _get_vectorized_flag():
        # Relocate tokens that ended up in the main card list
        ...
        # Manual overrides (must be last in card processing)
        apply_manual_overrides(mtgjson_set.cards)

    # =========================================================================
    # SET-LEVEL OPERATIONS (always executed)
    # These are not card-specific and apply regardless of vectorized mode.
    # =========================================================================
    ...
    # Build tokens
    if not skip_tokens:
        # In vectorized mode, extra_tokens are stored in module variable
        # In legacy mode, relocate_miscellaneous_tokens sets mtgjson_set.extra_tokens
        extra_tokens = (
            _vectorized_extra_tokens
            if _get_vectorized_flag()
            else (mtgjson_set.extra_tokens or [])
        )
        if extra_tokens:
            mtgjson_set.tokens = build_base_mtgjson_tokens(f"T{set_code}", extra_tokens)
        else:
            mtgjson_set.tokens = []
        add_other_face_ids(mtgjson_set.tokens)
    else:
        LOGGER.info(f"Skipping token build for {set_code} (--skip-tokens flag)")
    ...
    # Booster and sealed product data
    ...
    # Token-specific operations (legacy path only)
    # In vectorized mode, signature/orientation are applied in the DataFrame pipeline
    if not skip_tokens and not _get_vectorized_flag():
...
def build_base_mtgjson_cards(
    ...
    if _get_vectorized_flag() and not additional_cards:
        from .card_pipeline import (
            ...
            build_set_cards,
            dataframe_to_card_objects,
            filter_out_tokens,
            link_foil_nonfoil_versions,
            select_output_columns,
        )

        LOGGER.info(f"Using vectorized pipeline for {set_code}")

        # Get set metadata for token-specific operations
        set_data = GLOBAL_CACHE.get_set(set_code.upper())
        ...
        # Apply post-collect DataFrame operations
        cards_df = add_other_face_ids(cards_df)
        cards_df = add_variations(cards_df, set_code=set_code)
        cards_df = add_rebalanced_linkage(cards_df)
        cards_df = link_foil_nonfoil_versions(cards_df, set_code)
        cards_df = add_meld_card_parts(cards_df)
        cards_df = add_secret_lair_subsets(cards_df, set_code)
        cards_df = add_duel_deck_side(cards_df, set_code)
        cards_df = apply_manual_overrides(cards_df)
        cards_df = add_source_products(cards_df)
        cards_df = add_purchase_urls_struct(cards_df)
        cards_df = add_multiverse_bridge_ids(cards_df)

        # Token-specific operations (applies to both cards and tokens)
        cards_df = add_token_signatures(cards_df, set_name, set_type, set_code)
        cards_df = add_orientations(cards_df, set_code, set_name)

        # Separate tokens from cards (tokens may have ended up in main card list)
        cards_df, tokens_df = filter_out_tokens(cards_df)

        # Store token scryfall objects for later token building
        global _vectorized_extra_tokens
        if not tokens_df.is_empty() and "identifiers" in tokens_df.columns:
            # Extract scryfall IDs from identifiers struct
            token_ids = (
                tokens_df
                .select(pl.col("identifiers").struct.field("scryfallId"))
                .to_series()
                .to_list()
            )
            _vectorized_extra_tokens = GLOBAL_CACHE.cards_df.filter(
                pl.col("id").is_in(token_ids)
            ).to_dicts()
        else:
            _vectorized_extra_tokens = []

        # Clean up internal columns (underscore-prefixed and intermediate CK URL columns)
        internal_cols = [c for c in cards_df.columns if c.startswith("_")]
        # Also drop CardKingdom URL columns used for purchase URL generation
        ck_url_cols = ["cardKingdomUrl", "cardKingdomFoilUrl", "cardKingdomEtchedUrl"]
        internal_cols.extend([c for c in ck_url_cols if c in cards_df.columns])
        if internal_cols:
            cards_df = cards_df.drop(internal_cols)

        # Convert DataFrame to MtgjsonCardObject list for compatibility
        mtgjson_cards = dataframe_to_card_objects(cards_df, set_code, is_token)

        # Ensure consistent ordering
        mtgjson_cards.sort()

        LOGGER.info(f"Finished building {len(mtgjson_cards)} cards for {set_code}")
        return mtgjson_cards

    # Legacy path: per-card Python iteration
    # Get English cards for this set from cache
    # Note: Scryfall stores set codes in lowercase
    cards = GLOBAL_CACHE.cards_df.filter(
        (pl.col("set") == set_code.lower()) & (pl.col("lang") == "en")
    ).to_dicts()
...
def build_base_mtgjson_cards_vectorized(
    set_code: str,
    set_release_date: str = "",
) -> pl.DataFrame:
    """
    Build cards using the vectorized Polars pipeline.

    This is an alternative implementation that uses LazyFrame operations
    instead of per-card Python iteration. Returns a DataFrame for
    testing/comparison with the original implementation.

    :param set_code: Set code to build (e.g., "10E", "MH3")
    :param set_release_date: Original set release date for comparison
    :return: DataFrame with all card data
    """
    from .card_pipeline import (
        add_other_face_ids,
        add_variations,
    ...
    # Build cards using the pipeline
    cards_lf = build_set_cards(set_code, set_release_date)

    # Select output columns and collect
    cards_df = select_output_columns(cards_lf).collect()

    # Apply post-collect operations
    cards_df = add_other_face_ids(cards_df)
    cards_df = add_variations(cards_df, set_code=set_code)
    ...
    # Sort by name and number for consistent output
    cards_df = cards_df.sort(["name", "number"])

    LOGGER.info(f"Finished building {len(cards_df)} cards for {set_code}")
    return cards_df
...
def add_is_starter_option(
    ...
    starter_ids = set(
        GLOBAL_CACHE.cards_df.filter(
            (pl.col("set") == set_code.lower()) & ~pl.col("booster")
        )["id"].to_list()
    )
        ...
        if mtgjson_set.release_date > "2019-10-01":
            ...
            if GLOBAL_CACHE.cards_df is not None:
                base_set_count = GLOBAL_CACHE.cards_df.filter(
                    (pl.col("set") == mtgjson_set.code.lower())
                    & (pl.col("lang") == "en")
                    & (pl.col("booster"))
                ).height
