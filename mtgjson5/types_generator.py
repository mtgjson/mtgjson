...
class TypeScriptGenerator:
    ...
    def python_type_to_ts(self, field_info: FieldInfo, field_type: Any) -> str:
        ...
        if origin is Union:
            ...
            return " | ".join(
                self.python_type_to_ts(field_info, arg) for arg in non_none_args
            )
        ...
        if result == "any" and type_name not in ["Any"]:
            print(
                f"⚠ Warning: Unmapped type '{type_name}' for field, defaulting to 'any'"
            )

        return result

    def generate_interface(
        self, model: Any, name_override: Optional[str] = None
    ) -> str:
        """Generate TypeScript interface from Pydantic model"""
        if model is None:
            return ""

        interface_name = name_override or model.__name__.replace("Mtgjson", "").replace(
            "Object", ""
        )
