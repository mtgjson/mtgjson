...
class PolarsPriceBuilder:
    ...
    def stream_write_prices_sqlite(self, lf: pl.LazyFrame, path: Path) -> None:
        """
        Stream-write price data to a SQLite binary database using prefix partitioning.

        This memory-efficient implementation processes data in chunks by UUID
        prefix (0-9, a-f = 16 chunks), never materializing the full dataset.
        """
        from mtgjson5.classes import MtgjsonMetaObject
        ...
        LOGGER.info(f"Streaming {path.name} using prefix partitioning")
        ...
        for prefix in prefixes:
            chunk_lf = lf.filter(pl.col("uuid").str.starts_with(prefix))
            ...
            # Prepare chunk for SQL (rename columns)
            prepared = self._prepare_price_df_for_sql(df_chunk)
            rows = prepared.rows()
            chunk_rows = len(rows)
            ...
            LOGGER.info(
                f"  Processed prefix '{prefix}' ({chunk_rows:,} rows, "
                f"total: {total_rows:,})"
            )
        ...
        conn.commit()
        conn.close()
        LOGGER.info(f"Wrote {path.name} ({total_rows:,} rows)")
    ...
    def stream_write_prices_sql(self, lf: pl.LazyFrame, path: Path) -> None:
        """
        Stream-write price data as a SQL text dump using prefix partitioning.
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            # Prepare column names for INSERT statements
            col_names = ", ".join([f'"{c}"' for c in self._PRICE_SQL_COLUMNS])
    ...
    def stream_write_prices_psql(self, lf: pl.LazyFrame, path: Path) -> None:
        """
        Stream-write price data as a PostgreSQL dump using prefix partitioning.
    ...
    def build_prices(self) -> tuple[Path | None, Path | None]:
        ...
        # Write SQL formats for AllPrices (streaming to avoid OOM)
        LOGGER.info("Streaming AllPrices SQL formats")
        self.stream_write_prices_sqlite(archive_lf, output_path / "AllPrices.sqlite")
        self.stream_write_prices_sql(archive_lf, output_path / "AllPrices.sql")
        self.stream_write_prices_psql(archive_lf, output_path / "AllPrices.psql")
