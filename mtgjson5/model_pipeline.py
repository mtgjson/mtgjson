...
def build_assembly_context(ctx: PipelineContext) -> AssemblyContext:
    ...
    # Note: Use raw ctx.decks_df instead of build_expanded_decks_df() to keep minimal {count, uuid, isFoil}
...
def build_deck_files(
    ...
    # Get expanded decks
    decks_df = build_expanded_decks_df(ctx, set_codes=ctx.sets_to_build)
    if isinstance(decks_df, pl.LazyFrame):
        decks_df = decks_df.collect()
