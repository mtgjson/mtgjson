...
from mtgjson5.models.projections.mtgjson import (
    ALL_CARD_FIELDS,
    ATOMIC_EXCLUDE,
    CARD_DECK_EXCLUDE,
    CARD_SET_EXCLUDE,
...
#
#    schema = lf.collect_schema() # metadata only, no materialization
#    if "card_faces" not in schema.names():
...
#        )
#
#    # Generate indices matching list length; null lists produce null
#    lf = lf.with_columns(
#        pl.int_ranges(pl.col("card_faces").list.len()).alias("_face_idx")
#    )
#
#    # Explode both together — null lists become single rows with null values
#    lf = lf.explode(["card_faces", "_face_idx"])
#
#
...
# 1.1a: Expression to detect tokens
...
# 1.1d: Shorthand for card-only or token-only expressions
...
# 1.1e: Shorthand for token-only expressions
...
def add_legalities_struct(
    lf: pl.LazyFrame, ctx: Optional[PipelineContext] = None
) -> pl.LazyFrame:
    """
...
# 5.7:
def add_multiverse_bridge_ids(
    df: pl.LazyFrame, ctx: Optional[PipelineContext] = None
) -> pl.LazyFrame:
    """
