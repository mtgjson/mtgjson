...
_CARD_FACES = pl.List(pl.Struct({"oracle_id": pl.String}))
...
class TestExtractTokenReferences:
    ...
    def test_falls_back_to_the_face_oracle_of_a_reversible_token(self):
        """Reversible printings carry no top-level oracle_id, only face ones."""
        cards = pl.LazyFrame(
            [
                {"id": CARD, "oracle_id": "card-oracle", "all_parts": _token(OLD_PRINTING), "card_faces": None},
                {
                    "id": OLD_PRINTING,
                    "oracle_id": None,
                    "all_parts": None,
                    "card_faces": [{"oracle_id": ORACLE}, {"oracle_id": ORACLE}],
                },
            ],
            schema={
                "id": pl.String,
    ...
    def test_skips_cards_whose_layout_never_publishes_tokens(self):
        """Token-layout cards get a null relatedCards.tokens, so pins are dead weight."""
        cards = pl.LazyFrame(
            [
                {"id": CARD, "oracle_id": "card-oracle", "all_parts": _token(OLD_PRINTING), "layout": "token"},
                {"id": OLD_PRINTING, "oracle_id": ORACLE, "all_parts": None, "layout": "token"},
            ],
            schema={"id": pl.String, "oracle_id": pl.String, "all_parts": _ALL_PARTS, "layout": pl.String},
        )

        assert extract_token_references(cards).height == 0

    def test_leaves_two_printings_of_one_oracle_unpinned(self):
        """The pin layout holds one printing per (card, oracle); pinning would drop the other."""
        cards = _cards_lf(
            [
                {
                    "id": CARD,
                    "oracle_id": "card-oracle",
                    "all_parts": [
                        {"component": "token", "id": OLD_PRINTING},
                        {"component": "token", "id": NEW_PRINTING},
                    ],
                },
                {"id": OLD_PRINTING, "oracle_id": ORACLE, "all_parts": None},
    ...
    def test_a_repeated_reference_is_pinned_once(self):
        cards = _cards_lf(
            [
                {
                    "id": CARD,
        ...
        assert extract_token_references(cards).to_dicts() == [
            {"_card_sid": CARD, "_tok_oracle": ORACLE, "_tok_sid": OLD_PRINTING}
        ]
...
class TestBuildPins:
    ...
    def test_seeds_unpinned_references(self):
        merged = build_pins(self._refs(OLD_PRINTING), {}, {OLD_PRINTING})

        assert merged.pins == {CARD: {ORACLE: OLD_PRINTING}}
        assert (merged.added, merged.repointed) == (1, 0)
    ...
    def test_existing_pin_survives_a_scryfall_repoint(self):
        ...
        merged = build_pins(refs, existing, {OLD_PRINTING, NEW_PRINTING})

        assert merged.pins[CARD][ORACLE] == OLD_PRINTING
        assert merged.repointed == 0
    ...
    def test_pin_survives_when_no_card_references_it_any_more(self):
        ...
        merged = build_pins(self._refs(NEW_PRINTING), existing, {OLD_PRINTING, NEW_PRINTING})
    ...
    def test_pin_to_a_retired_printing_is_repointed(self):
        ...
        merged = build_pins(self._refs(NEW_PRINTING), existing, {NEW_PRINTING})

        assert merged.pins[CARD][ORACLE] == NEW_PRINTING
        assert merged.repointed == 1

    def test_round_trip_is_idempotent(self, tmp_path):
        first = build_pins(self._refs(OLD_PRINTING), {}, {OLD_PRINTING}).pins
        path = tmp_path / "pins.json"
        dump_pins(first, path)

        again = build_pins(self._refs(OLD_PRINTING), load_pins(path, refresh=True), {OLD_PRINTING})
        assert again.pins == first
        assert (again.added, again.repointed) == (0, 0)
...
class TestAddTokenIds:
    def _pins(self, pin_map: dict, uuid_map: pl.LazyFrame | None = None) -> object:
        cards = _cards_lf(
            [
                {"id": OLD_PRINTING, "oracle_id": ORACLE, "all_parts": None},
                {"id": NEW_PRINTING, "oracle_id": ORACLE, "all_parts": None},
        ...
        if uuid_map is None:
            uuid_map = _uuid_map(OLD_PRINTING, NEW_PRINTING)
        return build_token_pins(cards, uuid_map, pin_map)
    ...
    def test_pin_to_a_vanished_printing_falls_back_to_live(self):
        ...
        pins = self._pins({CARD: {ORACLE: OLD_PRINTING}}, _uuid_map(NEW_PRINTING))
    ...
    def test_empty_pin_file_disables_pinning(self):
        assert build_token_pins(_cards_lf([]), _uuid_map(), {}) is None
    ...
    def test_pins_apply_to_the_camel_cased_pipeline_frame(self):
        ...
        pins = build_token_pins(cards, _uuid_map(OLD_PRINTING, NEW_PRINTING), {CARD: {ORACLE: OLD_PRINTING}})
    ...
    def test_both_printings_survive_when_the_pair_is_unpinnable(self):
        """The ambiguous case extract_token_references() refuses to pin."""
        card = pl.LazyFrame(
            [
                {
                    "uuid": CARD_UUID,
        ...
        result = _resolve(card, _uuid_map(OLD_PRINTING, NEW_PRINTING), pins)

        assert result == [f"uuid-of-{OLD_PRINTING[:8]}", f"uuid-of-{NEW_PRINTING[:8]}"]
