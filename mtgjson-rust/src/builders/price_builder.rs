...
impl PriceBuilder {
    ...
    pub fn build_today_prices(&self) -> PyResult<HashMap<String, Py<PyAny>>> {
        Python::with_gil(|py| {
                    ...
                    Ok(provider_result) => {
                        ...
                        if let Ok(dict) = provider_result.downcast_bound::<pyo3::types::PyDict>(py)
                        {
                            for (key, value) in dict.iter() {
                                final_results.insert(key.extract::<String>()?, value.unbind());
                            }
                        }
                    }
                    Err(e) => {
    ...
    pub fn build_prices(&self) -> PyResult<(HashMap<String, Py<PyAny>>, HashMap<String, Py<PyAny>>)> {
