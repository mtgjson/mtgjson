...
mod tests {
    ...
    fn test_deck_add_sealed_product_uuids() {
        ...
        assert_eq!(deck.sealed_product_uuids.unwrap(), vec!["test-uuid".to_string()]);
    }
    ...
    fn test_deck_populate_deck_from_api() {
        ...
        assert!(deck.sealed_product_uuids.is_some());
        assert_eq!(deck.sealed_product_uuids.unwrap(), vec!["deck-uuid".to_string()]);
    }
    ...
    fn test_deck_get_total_cards() {
        ...
        deck.display_commander.push(r#"{"name": "Display"}"#.to_string());
    ...
    fn test_deck_sealed_product_uuid_matching() {
        ...
        assert!(deck.sealed_product_uuids.is_some());
        assert_eq!(deck.sealed_product_uuids.unwrap(), vec!["lightning-uuid".to_string()]);
    }
    ...
    fn test_deck_multiple_card_types() {
        ...
        deck.display_commander.push(r#"{"name": "Display Commander", "count": 1}"#.to_string());
        
        for i in 0..10 {
            deck.planes.push(format!(r#"{{"name": "Plane {}", "count": 1}}"#, i));
        }
        
        for i in 0..20 {
            deck.schemes.push(format!(r#"{{"name": "Scheme {}", "count": 1}}"#, i));
        }
    ...
    fn test_deck_complex_integration_scenario() {
        ...
        // Add commander
        deck.add_commander_card(r#"{"name": "Atraxa, Praetors' Voice", "count": 1, "uuid": "atraxa-uuid"}"#.to_string());
        deck.display_commander.push(r#"{"name": "Atraxa, Praetors' Voice", "count": 1}"#.to_string());
        
        // Add main deck cards (99 cards for EDH)
        let card_names = vec![
            "Sol Ring", "Command Tower", "Cultivate", "Kodama's Reach", "Counterspell",
            "Swords to Plowshares", "Path to Exile", "Toxic Deluge", "Wrath of God", "Cyclonic Rift",
            "Mystical Tutor", "Enlightened Tutor", "Vampiric Tutor", "Demonic Tutor", "Green Sun's Zenith",
            "Chord of Calling", "Craterhoof Behemoth", "Avenger of Zendikar", "Elspeth, Knight-Errant",
            "Jace, the Mind Sculptor", "Liliana of the Veil", "Garruk Wildspeaker", "Doubling Season",
            "Parallel Lives", "Anointed Procession", "Hardened Scales", "Cathars' Crusade", "Master Biomancer",
            "Forgotten Ancient", "Deepglow Skate", "Vorel of the Hull Clade", "Ezuri, Claw of Progress"
        ];
        ...
        assert!(deck.sealed_product_uuids.is_some());
        assert_eq!(deck.sealed_product_uuids.unwrap(), vec!["c16-breed-lethality".to_string()]);
        ...
        assert_eq!(header_info.get("name"), Some(&"Atraxa, Praetors' Voice EDH".to_string()));
    ...
    fn test_deck_with_all_card_types_populated() {
        ...
        deck.display_commander.push(r#"{"name": "DisplayCmd"}"#.to_string());
