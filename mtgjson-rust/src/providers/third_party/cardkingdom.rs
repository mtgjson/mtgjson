...
impl CardKingdomProvider {
    ...
    pub fn generate_today_price_dict(
        ...
        runtime
            .block_on(async {
                self.generate_today_price_dict_async(all_printings_path)
                    .await
            })
            .map_err(|e| {
    ...
    pub fn update_sealed_urls(
        &self,
        mut sealed_products: Vec<MtgjsonSealedProductObject>,
    ) -> PyResult<Vec<MtgjsonSealedProductObject>> {
        let runtime = tokio::runtime::Runtime::new()?;
        runtime
            .block_on(async {
...
impl CardKingdomProvider {
    ...
    async fn generate_today_price_dict_async(
        &self,
        all_printings_path: &str,
    ) -> ProviderResult<HashMap<String, MtgjsonPricesObject>> {
        let api_response: Value = self.download(Self::API_URL, None).await?;
        let empty_vec: Vec<Value> = vec![];
        let price_data_rows: Vec<Value> = api_response
            .get("data")
        ...
        let mut card_kingdom_id_to_mtgjson: HashMap<String, HashSet<String>> = self
        ...
        // Add foil IDs
        let foil_mapping: HashMap<String, HashSet<String>> = self
            .generate_entity_mapping(
                all_printings_path,
        ...
        // Add etched IDs
        let etched_mapping: HashMap<String, HashSet<String>> = self
            .generate_entity_mapping(
                all_printings_path,
        ...
        let default_prices_obj: MtgjsonPricesObject = MtgjsonPricesObject {
    ...
    async fn update_sealed_urls_async(
        &self,
        sealed_products: &mut Vec<MtgjsonSealedProductObject>,
    ) -> ProviderResult<()> {
        let api_data: Value = self.download(Self::SEALED_URL, None).await?;
        let empty_vec: Vec<Value> = vec![];
        let data_array: &[Value] = api_data
            .get("data")
            .and_then(|v| v.as_array())
            .unwrap_or(&empty_vec);
                        ...
                        if let Some(remote_id) = remote_product.get("id").and_then(|v| v.as_str()) {
                            if remote_id == card_kingdom_id {
                                if let (Some(base_url), Some(url_path)) = (
                                    ...
                                    let mut purchase_urls =
                                        crate::classes::MtgjsonPurchaseUrls::new();
    ...
    async fn generate_entity_mapping(
        ...
        Ok(HashMap::<String, HashSet<String>>::new())
    }
}
...
impl AbstractProvider for CardKingdomProvider {
    ...
    fn generic_generate_today_price_dict(
        ...
        for data_row in price_data_rows {
            let third_party_id = data_row
                .get(card_platform_id_key)
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string();
            ...
            if let Some(mtgjson_uuids) = third_party_to_mtgjson.get(&third_party_id) {
                for mtgjson_uuid in mtgjson_uuids {
                    ...
                    let prices: &mut MtgjsonPricesObject = today_dict
