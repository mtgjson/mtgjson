...
impl WhatsInStandardProvider {
    ...
    #[pyo3(signature = ())]
    pub fn standard_legal_set_codes(&self) -> PyResult<PyObject> {
        Python::with_gil(|py| {
        ...
        let sets = rt
            .block_on(async { self.standard_legal_set_codes_async().await })
            .map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                    "Standard sets fetch error: {}",
                    e
                ))
            })?;
        ...
        Python::with_gil(|py| {
            ...
            Ok(py_set.into_py(py))
        })
    }

    pub fn refresh_cache(&self) -> PyResult<PyObject> {
        {
            let mut cache = STANDARD_SETS_CACHE.write().map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                    "Cache lock error: {}",
        ...
        // Fetch fresh data
        self.standard_legal_set_codes()
    }

    pub fn is_set_standard_legal(&self, set_code: &str) -> PyResult<bool> {
        let standard_sets = self.standard_legal_set_codes()?;
        Python::with_gil(|py| {
            let py_set = standard_sets.downcast_bound::<PySet>(py)?;
            Ok(py_set.contains(set_code)?)
        })
    }

    #[staticmethod]
    pub fn get_cache_stats() -> PyResult<PyObject> {
        let cache = STANDARD_SETS_CACHE.read().map_err(|e| {
...
impl WhatsInStandardProvider {
            ...
            match self.fetch_standard_sets_attempt().await {
                ...
                Err(e) => {
                    ...
                    if attempt < Self::MAX_RETRIES - 1 {
                        let delay =
                            Duration::from_millis(Self::RETRY_DELAY_MS * (2_u64.pow(attempt)));
    ...
    async fn fetch_standard_sets_attempt(&self) -> ProviderResult<FxHashSet<String>> {
        ...
        let sets_array = api_response
            .get("sets")
            .and_then(|v| v.as_array())
            .ok_or_else(|| {
                ProviderError::ParseError("Missing 'sets' array in API response".to_string())
            })?;
...
mod tests {
    ...
    fn test_python_api_compatibility() {
        Python::with_gil(|py| {
            ...
            let result = provider.standard_legal_set_codes();
            assert!(result.is_ok());
            ...
            let is_legal = provider.is_set_standard_legal("DOM");
    ...
    fn bench_date_parsing(b: &mut test::Bencher) {
        b.iter(|| WhatsInStandardProvider::parse_date_optimized("2023-10-15"));
    }

    #[bench]
    fn bench_standard_check(b: &mut test::Bencher) {
        let now = Utc::now();
        b.iter(|| WhatsInStandardProvider::is_set_currently_standard("2023-01-01", "9999", &now));
    }
}
