use super::{
    super::{AbstractProvider, BaseProvider, ProviderError, ProviderResult},
    sf_utils,
};
...
impl ScryfallProviderOrientationDetector {
    ...
    pub fn get_uuid_to_orientation_map(&self, set_code: &str) -> PyResult<HashMap<String, String>> {
        ...
        runtime
            .block_on(async { self.get_uuid_to_orientation_map_async(set_code).await })
            .map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                    "Orientation map error: {}",
                    e
                ))
            })
    }
...
impl ScryfallProviderOrientationDetector {
    async fn get_uuid_to_orientation_map_async(
        &self,
        set_code: &str,
    ...
    fn parse_orientation(&self, orientation_header: &scraper::ElementRef) -> String {
        if let Some(link) = orientation_header
