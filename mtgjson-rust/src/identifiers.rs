...
//! MtgjsonIdentifiers
//!
//! This struct represents the identifiers for a single Magic: The Gathering card.
//! It is used to store all the data for a single card, including
//! its identifiers, such as card kingdom etched id, card kingdom foil id,
//! card kingdom id, cardsphere foil id, cardsphere id, mcm id, mcm meta id,
//! mtg arena id, mtgjson foil version id, mtgjson non foil version id,
//! mtgjson v4 id, mtgo foil id, mtgo id, multiverse id, scryfall id,
//! scryfall illustration id, scryfall card back id, scryfall oracle id,
//! tcgplayer etched product id, and tcgplayer product id.
//!
