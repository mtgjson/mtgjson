...
class Gatherer:
    ...
    def parse_cards(self, gatherer_data: str) -> List[GathererCard]:
        ...
        return [self._parse_column(c) for c in columns]

    def _parse_column(self, gatherer_column: bs4.element.Tag) -> GathererCard:
    ...
    def _replace_symbols(self, tag: bs4.BeautifulSoup) -> bs4.BeautifulSoup:
        """Replace all image tags with their mapped symbol."""
        tag_copy = copy.copy(tag)
        images = tag_copy.find_all("img")
        for image in images:
            alt = image["alt"]
            symbol = self.symbol_map.get(alt, alt)
