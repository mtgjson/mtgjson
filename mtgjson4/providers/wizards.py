...
class Wizards:
    ...
    def get_comp_rules(self) -> str:
        ...
        """
        response = self.download_from_wizards(self.comp_rules)
        ...
        response = self.download_from_wizards(comp_rules_url)

        return response

    def compile_comp_output(self) -> Dict[str, List[str]]:
        """
        ...
        """
        comp_rules = self.get_comp_rules()

        return dict(
            AbilityWords=self.get_ability_words(comp_rules),
            KeywordActions=self.get_keyword_actions(comp_rules),
            KeywordAbilities=self.get_keyword_abilities(comp_rules),
        )
