...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        ...
        nargs=1,
        help=
        'You can limit how many sets will be built at one time. The higher the number, the more memory consumption. '
        'If not enough memory, swap space will be used, which slows down the program tremendiously. Defaults to 5. 0 '
        'to Disable. '
    )
    ...
    arg_parser = vars(arg_parser.parse_args())
    lang_to_process = arg_parser['language'][0]
    ...
    if (arg_parser['sets'] is None) and (not arg_parser['all_sets']) and arg_parser['full_out']:
        mtg_builder.create_combined_outputs()
        exit(0)

    # Global of all sets to build
    SETS_TO_BUILD = mtg_builder.determine_gatherer_sets(arg_parser)
    ...
    card_loop.run_until_complete(main(card_loop, card_session, lang_to_process, arg_parser))

    if arg_parser['full_out']:
        mtg_builder.create_combined_outputs()
